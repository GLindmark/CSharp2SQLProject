...
namespace CSharp2SQLProject {
    class Program {
        static void Main(string[] args) {
            ...
            var ord = GetOrderbyID(1024);
            if (ord == null) {
                Console.WriteLine("Customer not found");

            }
            else {
                Console.WriteLine($"Order Id: {orders.Id}| {orders.Date}");
            }
        }
        /*
        ...
        static Customer GetCustomerbyID(int pid) {
            ...
            cmd.Parameters.Add(theId);       //allows for adding parameters to the program;
            ...
            if (reader.Read()) {
                ...
                reader.Close();//closed out before the return
                connection.Close();//closed the connection before the return
        ...
        static Order GetOrderbyID(int pid) {
            ...
            if (reader.Read()) {
                ...
                var note = reader["Note"].ToString();
                var customer = reader["CustomerId"].ToString();
                ord = new Order(id, date, note, customer);
